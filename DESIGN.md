...
- **Readable passages** (`renderLine`): every line of Arabic or translation is addressable. The line takes its own `tabindex` and keeps `cursor` in CSS, deliberately — an inline one would make `enhanceTappables` announce every verse as "button". Tapping or focusing a line renders its action strip (bookmark, favourite, 40px each); the strip is only in the DOM where it is wanted, because Jawshan Kabeer is 1300 lines. A saved line keeps its strip visible, which is how "active state" reads.
- **Provenance labels.** Anywhere prayer times appear, `prayerSource()` decides what the surface admits: `live` (fetched today), `cached` (the last timetable saved for this town), `preset` (the centre's own Dublin timetable) or `fallback` (Dublin's times standing in for a town that has none yet). `fallback` is always labelled in a warning tone — showing one town's times under another town's name is the one failure this app must never make quietly.
...
- **Local data.** Preferences go to `localStorage` via `lsGet`/`lsSet`. Bookmarks and favourites go to **IndexedDB** (`abi-saved`, store `marks`), because they grow without limit; `localStorage` holds only a mirror so the first paint has rows and the feature degrades where IDB is refused. `_saved` on the component is the synchronous copy every writer reads — `setState` is batched, and two saves in one tick would otherwise both start from the same stale array. Passage identity is `abiHash(text)`, never an array index, so a mark survives the library being re-ordered upstream; `serialiseSaved()` is the shape a backup would write.
